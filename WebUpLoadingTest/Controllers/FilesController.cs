...
public class FilesController : Controller
{
    ...
    public async Task<IActionResult> UploadSingle(CancellationToken Cancel)
    {
        ...
        while (await reader.ReadNextSectionAsync(Cancel) is { Body: var body } section)
            if (ContentDispositionHeaderValue.TryParse(section.ContentDisposition, out var position))
            {
                ...
                try
                {
                    ...
                    do
                    {
                        readed       =  await body.ReadAsync(buffer, 0, buffer.Length, Cancel);
                        total_readed += readed;
                        //if (i++ % buffer.Length == 0)
                        //    _Logger.LogInformation("Прочитано {0:f2} MБайт", total_readed / 1048576f);
                        //if (i == 100) await Task.Delay(5000);

                        // ЗДЕСЬ ДОЛЖНА ВЫПОЛНЯТЬСЯ ОБРАБОТКА ФАЙЛА, ИЛИ ЗАПИСЬ ЕГО НА ДИСК
                    }
                    while (readed > 0);
